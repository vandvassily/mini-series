...
function createElement(tag, attrs) {
  ...
  var key = attrs.key || null; // 如果元素上有key，则去除key，如果没有则设置为null
  ...
  return {
    // 创建一个普通JavaScript对象，并将各属性添加上去，作为虚拟DOM进行返回
    key: key,
    tag: tag,
    attrs: attrs,
    children: children
...
function render(vnode, container) {
  if (vnode.isReactComponent) {
    container.innerHTML = ''; // 如果是直接渲染类组件
  ...
  if (typeof vnode.tag === 'function' && !vnode.isReactComponent) {
    ...
    if (vnode.tag.prototype && vnode.tag.prototype.render) {
      ...
      _component._container = container; // 根据类组件名创建组件实例并调用render()函数拿到对应的虚拟DOM节点
  ...
  if (typeof vnode === 'string' || typeof vnode === 'number') {
    var textNode = document.createTextNode(vnode); // 直接创建一个文本节点
  ...
  var element = document.createElement(vnode.tag); // 处理属性
...
var Component = /*#__PURE__*/function () {
  ...
  _createClass(Component, [{
    ...
    value: function setState(newState) {
      ...
      _reactDom.default.render(this, this._container); // 重新渲染组件
